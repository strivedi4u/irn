...
@Service
public class TableKafkaDmsConsumer {

    private final AtomicReference<String> latestData = new AtomicReference<>("[]");

    @KafkaListener(topics = "oracle-table-dms-topic", groupId = "irn-dms-group")
    ...
    public String getLatestData() {
        try {
            ...
            if (data == null || data.trim().isEmpty() || "[]".equals(data.trim())) {
                String backup = TableKafkaDmsProducer.getLatestBackupJson();
