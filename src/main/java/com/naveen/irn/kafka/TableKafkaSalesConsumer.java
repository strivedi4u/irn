...
@Service
public class TableKafkaSalesConsumer {

    private final AtomicReference<String> latestData = new AtomicReference<>("[]");

    @KafkaListener(topics = "oracle-table-sales-topic", groupId = "irn-sales-group")
    ...
    public String getLatestData() {
        try {
            ...
            if (data == null || data.trim().isEmpty() || "[]".equals(data.trim())) {
                String backup = TableKafkaSalesProducer.getLatestBackupJson();
