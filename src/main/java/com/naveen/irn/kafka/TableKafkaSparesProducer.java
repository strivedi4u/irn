...
@Service
public class TableKafkaSparesProducer {

    private static final String TOPIC = "oracle-table-spares-topic";
    ...
    public void fetchAndSendTableData() {
        System.out.println("Scheduled fetchAndSendTableData started (Spares).");
        if (!isKafkaAvailable()) {
            System.out.println("Kafka broker is not available at localhost:9092. Skipping send. Please start your local Kafka server.");
            return;
        }
        String sql = IrnSqls.SPARES_IRN_SQL;
        System.out.println("Executing SQL for Kafka Spares producer: " + sql);
        List<Map<String, Object>> rows = JdbcTemplate.queryForList(sql);
        System.out.println("Fetched " + rows.size() + " rows from DB for Kafka (Spares).");
        try {
            String json = objectMapper.writeValueAsString(rows);
            System.out.println("Sending data to Kafka topic: " + TOPIC);
            kafkaTemplate.send(TOPIC, json).get();
            latestBackupJson = json;
            System.out.println("Data sent to Kafka and backup updated (Spares).");
        } catch (Exception e) {
            System.out.println("Error sending data to Kafka, sending empty array (Spares).");
            e.printStackTrace();
            try {
                kafkaTemplate.send(TOPIC, "[]").get();
            } catch (Exception ex) {
                System.out.println("Failed to send empty array to Kafka (Spares).");
                ex.printStackTrace();
            }
            latestBackupJson = "[]";
        }
    ...
    public void createTopicIfNotExists() {
        ...
        try {
            ...
            if (isKafkaAvailable()) {
                System.out.println("Topic may already exist or error occurred during topic creation (Spares).");
                e.printStackTrace();
            } else {
                System.out.println("Kafka broker became unavailable during topic creation (Spares). Skipping stack trace.");
            }
        }
    }
}
