...
@Service
public class TableKafkaMaterialConsumer {

    private final AtomicReference<String> latestData = new AtomicReference<>("[]");

    @KafkaListener(topics = "oracle-table-material-topic", groupId = "irn-material-group")
...
public String getLatestData() {
    try {
        ...
        if (data == null || data.trim().isEmpty() || "[]".equals(data.trim())) {
            String backup = TableKafkaMaterialProducer.getLatestBackupJson();
